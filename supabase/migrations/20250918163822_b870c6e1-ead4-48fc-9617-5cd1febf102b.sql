-- Fix the setup fee completion issue and prevent duplicate transactions
...
BEGIN
  ...
  IF p_order_status = 'meeting_successful' THEN
    ...
    LOOP
      -- Only insert if this exact transaction hasn't been processed yet
      IF NOT EXISTS (
  ...
  -- When client pays, complete all fees (each as separate transaction)
  ELSIF p_order_status = 'client_paid' THEN
    ...
    -- Process each fee type as separate transactions
    FOR v_transaction IN 
      SELECT * FROM order_transactions 
      WHERE order_id = p_order_id 
      AND transaction_type IN ('meeting_fee', 'setup_fee', 'automation_cost', 'followup_fee', 'dropservicing_fee')
  ...
  ELSIF p_order_status = 'order_completed' THEN
    ...
    -- Process payment (earnings) to wallet as a single deposit
    FOR v_transaction IN 
      SELECT * FROM order_transactions 
      WHERE order_id = p_order_id 
    ...
    -- Process any remaining fees that weren't processed yet as separate transactions
    FOR v_transaction IN 
      SELECT * FROM order_transactions 
      WHERE order_id = p_order_id 
      AND transaction_type IN ('meeting_fee', 'setup_fee', 'automation_cost', 'followup_fee', 'dropservicing_fee')
...
-- Add transaction_type column to transactions table to help prevent duplicates
